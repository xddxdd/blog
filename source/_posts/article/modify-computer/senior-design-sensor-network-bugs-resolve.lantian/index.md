...
受到我们的设备限制，我们只能假设空气中气体浓度不会变化太多，并以此忽略斜率的不同。对于基础阻值不同，我们将分压测阻值用的电阻从固定阻值换成了电位器（变阻器）。此时程序计算的就不是绝对的电阻数值，而是相对于基准值（即电位器设置的电阻）的比例。再将所有设备放在同一环境下，与其它专业设备和天气预报对比，调整电位器，使得所有 MiCS6814 的读数相同。
