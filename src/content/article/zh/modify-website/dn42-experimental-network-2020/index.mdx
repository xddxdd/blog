   ...
   4. 接下来就进入 IP 选择环节了。进入 `data/inetnum` 文件夹，里面是所有已被注册
      ...
      - DN42 的 IPv4 地址在 `172.20.0.0/14` 范围，即
        ...
        - **请使用下列工具选择地址块，而非手动在 `data/inetnum` 文件夹中挑选**。
          ...
          - [https://dn42.us/peers/free](https://dn42.us/peers/free)
      ...
      - 如果你的服务器**非常多（指超过 62 台）**，连 `/26` 都不够用，那么：
        ...
        - 然后你的注册申请不会立即被同意。此时你需要去以下任何一个地方，告诉大家
          ...
          - DN42 的 IRC 频道（服务器地址
            在[这个页面](https://dn42.dev/services/IRC)可以看到，其中的
            public internet 项）
          - DN42 的邮件列表（地址
            在[这个页面](https://dn42.dev/contact#contact_mailing-list)的
            Mailing list 项）
...
1. BIRD Internet Routing Daemon **(v2)**
   ...
   - 配置清晰明了，功能强大
    - 由于 BIRD 配置较复杂，请直接参见
      [DN42 Wiki 上的 Bird2 配置教程](https://dn42.dev/howto/Bird2)，有现成的
      配置可以直接复制粘贴。
...
我这里只介绍 BIRD v2 的配置，因为它是使用最广泛的。我假设你已经按照 Wiki
...
- 尝试
  [DN42 Wiki 上的内部服务](https://dn42.dev/internal/Internal-Services)，或
...
人会指责你。你可以在 DN42 的
[IRC 频道](https://dn42.dev/services/IRC)，[邮件列表](https://dn42.dev/contact#contact_mailing-list)或
