...
- 2026-08: Add the `max-length` attribute to the `route` / `route6` object examples; note that recent Windows 10/11 already comes with the `ssh` command built in.
...
[Joining DN42 Experimental Network](/en/article/modify-website/join-dn42-experimental-network.lantian),
...
4. Now, you need to create a series of files in the cloned repo, including:
   ...
   1. Create a file `[NICKNAME]-MNT` under `data/mntner` directory. This file is
      ...
      <InteractiveButtons
        vertical={true}
        buttons={[
          ['noop1', 'Here is my registration info for reference only.'],
          [
            'noop2',
            'Read the document carefully and replace EVERYTHING with YOUR information.',
          ],
          ['code1', 'I acknowledge the notes above, and want to see the code.'],
        ]}
      />
  ...
  - `auth`: your authentication info. Usually, two types of data are
    ...
    - If you plan to add an SSH pubkey, you need to create one first
      ...
      - Running `ssh-keygen -t ed25519` is usually enough for Mac and
        ...
        use RSA crypto.
      - Recent versions of Windows 10/11 already comes with the `ssh` command built in, so you can simply run
        `ssh-keygen -t ed25519` in PowerShell or CMD, just like on Linux,
        without installing PuTTY.
      - If your Windows version is older and does not have the `ssh`
        command, you can
        [download PuTTY](https://www.chiark.greenend.org.uk/~sgtatham/putty/latest.html)
        and use its `puttygen` utility as an alternative.
      - After generation, add your pubkey (usually stored at `~/.ssh` and
        named `id_ed25519.pub` or `id_rsa.pub` on with `ssh-keygen`; displayed
        straight on `puttygen` window after generation) to
        `auth` item, with format `ssh-ed25519 [SSH Pubkey]` or
        `ssh-rsa [SSH Pubkey]`.
   ...
   2. Create `[NICKNAME]-DN42` under `data/person`. This stands for your
      ...
      <InteractiveButtons
        ...
        buttons={[
          ...
          ['code2', 'I acknowledge the notes above, and want to see the code.'],
        ]}
      />
   ...
   3. Next, you need to choose for yourself an Autonomous System Number or ASN.
      ...
      <InteractiveButtons
        ...
        buttons={[
          ...
          ['code3', 'I acknowledge the notes above, and want to see the code.'],
        ]}
      />
   ...
   4. Next, you start to choose your IP. Browse to `data/inetnum` folder, where
      ...
      - If you **REALLY have a lot of servers (more than 62)** that doesn't fit
        ...
        - Your application won't be accepted immediately. In this case, you need
          ...
          - DN42's IRC channel ([here](https://dn42.dev/services/IRC), under
            `public internet`)
          - DN42's mailing list
            ([here](https://dn42.dev/contact#contact_mailing-list), under
            `Mailing list`)
      ...
      - When you finally finish choosing your IP block, create the file under
        ...
        <InteractiveButtons
          ...
          buttons={[
            ...
            [
              'code4',
              'I acknowledge the notes above, and want to see the code.',
            ],
          ]}
        />
   ...
   5. IP registration is not done yet. You also need a `route` object to
      ...
      <InteractiveButtons
        ...
        buttons={[
          ...
          ['code5', 'I acknowledge the notes above, and want to see the code.'],
        ]}
      />
...
- The items in the file are:
  ...
  - `max-length`: the maximum prefix length (i.e. the most specific subnet mask) that this AS is allowed to announce. Usually set to the same length as the prefix in `route`, meaning only this exact prefix may be announced. Increase it only when you need to announce smaller subnets carved out of a larger block.
  - `mnt-by`: `maintain by`, points to your `mntner` file,
    `[NICKNAME]-MNT`.
  - `source`: has a fixed value of `DN42`.
   ...
   7. Then create a `route6` object under `data/route6`, to authorize your AS to
      ...
      <InteractiveButtons
        ...
        buttons={[
          ...
          ['code7', 'I acknowledge the notes above, and want to see the code.'],
        ]}
      />
...
- The items in the file are:
  ...
  - `descr`: description, fill with anything you like.
  - `origin`: AS you authorize to use this IP block. Fill with your ASN.
  - `max-length`: the maximum prefix length (i.e. the most specific subnet mask) that this AS is allowed to announce. Usually set to the same length as the prefix in `route6`, meaning only this exact prefix may be announced. Increase it only when you need to announce smaller subnets carved out of a larger block.
  - `mnt-by`: `maintain by`, points to your `mntner` file,
    `[NICKNAME]-MNT`.
  - `source`: has a fixed value of `DN42`.
...
1. Use [DN42 PingFinder](https://peerfinder.dn42.dev/). Submit your IP address,
   and server nodes of other users registered on PingFinder will test their
   latency to you. These node entries usually have e-mail addresses or links to
   their description pages for their DN42 AS. You may contact them with this
   information.
2. Go to DN42's [IRC channel](https://dn42.dev/services/IRC).
...
1. BIRD Internet Routing Daemon **(v2)**
   ...
   - BIRD configuration is a bit complicated, so please refer to
     [DN42 Wiki's Bird2 Guide](https://dn42.dev/howto/Bird2), where a
...
2. FRRouting
   ...
   - Refer to [DN42 Wiki's FRRouting Guide](https://dn42.dev/howto/frr)
3. BGP functionality of hardware routers
...
## BGP Session Setup: BIRD v2

I will only talk about the configuration of BIRD v2 since it is the most widely
used. I assume you have finished the setup following the Wiki but don't have any
peers.

- [DN42 Wiki's Bird2 Guide](https://dn42.dev/howto/Bird2)

For BIRD v2, what you need is:

import Bird2 from '../../../../_templates/dn42-experimental-network-2020/bird2-en.md'
...
- Try
  [Internal Services on DN42 Wiki](https://dn42.dev/internal/Internal-Services),
...
[mailing list](https://dn42.dev/contact#contact_mailing-list) or the
